...
const Ex03 = () => {
    ...
    /* 실습) 랜덤 한 숫자를 맞추는 랜덤게임을 만들어보자
    1. 우리에게 필요한 state들을 미리 선언
     - 랜덤한 숫자 ranNum, 내가 선택한 숫자 myNum)
    2. 버튼을 누르면 ranNum에는 1~3중 랜덤한 값이 세팅되도록 할 것
      myNum에는 내가 누른 그 숫자가 세팅되도록 할 것(참고 : e.target.innerText)
    3. 랜덤한 수와 내가 누른 숫자가 같다면 '정답입니다'를 출력하고,
      다르다면 '땡...'을 출력
      ★ 주의사항 : if문 사용 X, 삼항연산자를 사용해서 return문 안에서 비교
   ...
   /* if문으로 해보기
    ...
    const gameStart = (e)=>{
        ...
        if(e.target.innerText == parseInt(Math.random()*3)+1){
            setMsg('정답')
        } else {
            setMsg('땡')
        }
...
*/
  ...
  return (
    <div>
        ...
        <hr></hr>
        <p>내가 입력 한 숫자 : {myNum}</p>
        <p>컴퓨터가 입력 한 숫자 : {ranNum}</p>
        <h3>{myNum == ranNum ? '정답!' : '땡!'}</h3>
        {/* <h3>{msg}</h3> */}


        {/* 
            1. 정답을 맞힐때마다 정답회숫 1증가
